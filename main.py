...
def detect_intent(project_id, session_id, text, language_code):
    session_client = dialogflow.SessionsClient.from_service_account_file(
        "datadelta-clarity-386808-09d602cc7b3f.json"
    )
...
@app.post("/dialogflow")
async def dialogflow(request: Request, db: Session = Depends(get_db)):
    ...
    # Call the OpenAI API to transcribe the audio using Whisper API
        ...
        Transcribed the voice note to the following text: {whisper_response}.
            Now it's being sent to Dialogflow API to reply...
        """)
    project_id = "delta-clarity-386808"
    import uuid

    session_id = str(uuid.uuid4())
    language_code = "en-US"  # Language code (e.g., en-US, en-GB)
    text = [{"role": "user", "content": whisper_response.get("text")}]

    dialogflowResponse = detect_intent(project_id, session_id, text, language_code)
    ...
    send_message(whatsapp_number, dialogflowResponse)
    return ""
...
async def reply(request: Request, db: Session = Depends(get_db)):
    ...
    messages.append({"role": "system", "content": " انت عامل في مطعم بيتزا وتريد ان تسجل طلبات العملاء في قائمه "})
